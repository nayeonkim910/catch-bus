...
export function FavoritesTab({ favorites, arrivals }: FavoritesTabProps) {
  return (
      ...
      {favorites.length > 0 ? (
        <div className="grid gap-3 lg:grid-cols-2 xl:grid-cols-3">
          {favorites.map((favorite) => (
            <FavoriteCard
              key={favorite.id}
              favorite={favorite}
              arrival={arrivals.find(
                (arrival) =>
                  arrival.stationId === favorite.stationId &&
                  arrival.routeId === favorite.routeId,
              )}
            />
          ))}
        </div>
        ...
        <div className="mx-auto max-w-sm rounded-2xl border border-dashed border-slate-300 bg-white/70 p-8 text-center">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-500">
            자주 타는 정류장과 버스를 저장하면 이 탭에서 빠르게 확인할 수
            있습니다.
          </p>
        </div>
      )}
