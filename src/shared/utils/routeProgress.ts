...
const STATION_GAP =
  (LAST_STATION_POSITION - FIRST_STATION_POSITION) /
  (ROUTE_PROGRESS_STATION_COUNT - 1)
...
export function getRouteProgressLayout(remainingStops: number | null) {
  ...
  const busPosition =
    LAST_STATION_POSITION - Math.max(remainingStops, 0) * STATION_GAP
...
export function getRouteProgressModel({
  ...
  return {
    ...
    destinationStationFallback: hasStationTimeline
      ? null
      : destinationStationName,
